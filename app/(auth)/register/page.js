...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center px-6 py-12" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm page-enter">
        <div className="mb-8">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Sudah punya akun?{' '}
            <Link href="/login" className="text-[var(--accent)] font-medium hover:underline">Masuk di sini</Link>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input label="Nama lengkap" type="text" placeholder="Muhammad Kelvin"
            value={form.name} onChange={set('name')} error={errors.name} icon={<User size={15} />} />
          <Input label="Email" type="email" placeholder="kamu@email.com"
            value={form.email} onChange={set('email')} error={errors.email} icon={<Mail size={15} />} />
          ...
          <Input label="Konfirmasi password" type="password" placeholder="Ulangi password"
            value={form.confirmPassword} onChange={set('confirmPassword')} error={errors.confirmPassword} icon={<Lock size={15} />} />
